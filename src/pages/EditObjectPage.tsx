...
function EditObjectPage() {
  ...
  useEffect(() => {
    async function loadObject() {
      if (!objectId) { setError('Keine Objekt-ID vorhanden.'); return }
      try {
        const loaded = await getObjectById(objectId)
        setObject(loaded)
        setHotspots(loaded.hotspots ?? [])
        setQuiz(loaded.quiz ?? [])
        if (loaded.imageUrl) setImagePreview(loaded.imageUrl)
      } catch {
  ...
  function handleImagePointerDown(e: PointerEvent<HTMLDivElement>) {
    ...
    // Bestehende Hotspots sollen anklickbar bleiben, ohne direkt einen neuen Hotspot zu erstellen.
    ...
    // Prozentwerte bleiben auch bei responsiven Bildgrössen korrekt.
  ...
  function updateHotspot(id: string, field: keyof Hotspot, value: string) {
    setHotspots((prev) => prev.map((hs) => hs.id === id ? { ...hs, [field]: value } : hs))
  }
  ...
  function addQuizQuestion() {
    setQuiz((prev) => [...prev, {
      id: `q-${Date.now()}`, questionDe: '', questionFr: '',
      answersDe: ['', '', ''], answersFr: ['', '', ''],
      correctAnswerIndex: 0, explanationDe: '', explanationFr: '',
    }])
  }

  function updateQuiz(id: string, field: keyof QuizQuestion, value: unknown) {
    setQuiz((prev) => prev.map((q) => q.id === id ? { ...q, [field]: value } : q))
  }
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = async (event) => {
    ...
    setError('')
    setMessage('')
    if (!object) { setError('Objekt konnte nicht aktualisiert werden.'); return }
    ...
    // Always send existing ID for updates
    ...
    return (
      <main className="app-page admin-page">
        <section className="card">
          ...
          <Link className="secondary-link" to="/admin">Zurück zur Übersicht</Link>
        </section>
      </main>
    )
  }
  ...
  if (!object) {
    return (
      <main className="app-page admin-page">
        <section className="card"><p>Objekt wird geladen...</p></section>
      </main>
    )
  }
    ...
    <main className="app-page admin-page">
      <section className="card">
        ...
        <form className="admin-form" onSubmit={handleSubmit}>
          ...
          {/* A) Grunddaten */}
          <div className="form-section-title form-field--full"><h2>A) Grunddaten</h2></div>

          <div className="form-field">
            <label htmlFor="titleDe">Titel Deutsch <span className="required-star">*</span></label>
            <input id="titleDe" name="titleDe" required defaultValue={object.titleDe} />
          </div>
          <div className="form-field">
            <label htmlFor="titleFr">Titel Französisch <span className="required-star">*</span></label>
            <input id="titleFr" name="titleFr" required defaultValue={object.titleFr} />
          </div>
          <div className="form-field form-field--full">
            <label htmlFor="shortDe">Kurzbeschreibung Deutsch <span className="required-star">*</span></label>
          ...
          {/* B) Medien */}
          <div className="form-section-title form-field--full"><h2>B) Medien</h2></div>
          ...
          {imagePreview && (
            <div className="form-field form-field--full">
              <p className="form-hint">Bild-Vorschau (klicken für Hotspot):</p>
          ...
          {/* C) Orientierung */}
          <div className="form-section-title form-field--full"><h2>C) Orientierung</h2></div>
          <div className="form-field form-field--full">
                ...
                <div key={hs.id} className="hotspot-form-item">
                  ...
                  <div className="admin-form">
                    <div className="form-field">
                      ...
                      <input id={`hs-tde-${hs.id}`} value={hs.titleDe} onChange={(e) => updateHotspot(hs.id, 'titleDe', e.target.value)} />
                    </div>
                    <div className="form-field">
                      <label htmlFor={`hs-tfr-${hs.id}`}>Titel FR</label>
                      <input id={`hs-tfr-${hs.id}`} value={hs.titleFr} onChange={(e) => updateHotspot(hs.id, 'titleFr', e.target.value)} />
                    </div>
                    <div className="form-field form-field--full">
                      <label htmlFor={`hs-xde-${hs.id}`}>Text DE</label>
                      <textarea id={`hs-xde-${hs.id}`} value={hs.textDe} onChange={(e) => updateHotspot(hs.id, 'textDe', e.target.value)} />
                    </div>
                    <div className="form-field form-field--full">
                      <label htmlFor={`hs-xfr-${hs.id}`}>Text FR</label>
                      <textarea id={`hs-xfr-${hs.id}`} value={hs.textFr} onChange={(e) => updateHotspot(hs.id, 'textFr', e.target.value)} />
                    </div>
                  </div>
          ...
          {/* E) Quiz */}
          <div className="form-section-title form-field--full"><h2>E) Quiz</h2></div>
              ...
              <div key={q.id} className="quiz-form-item">
                ...
                <div className="admin-form">
                  <div className="form-field form-field--full">
                    ...
                    <input id={`qde-${q.id}`} value={q.questionDe} onChange={(e) => updateQuiz(q.id, 'questionDe', e.target.value)} />
                  </div>
                  <div className="form-field form-field--full">
                    <label htmlFor={`qfr-${q.id}`}>Frage FR</label>
                    <input id={`qfr-${q.id}`} value={q.questionFr} onChange={(e) => updateQuiz(q.id, 'questionFr', e.target.value)} />
                  </div>
                  {(['A', 'B', 'C'] as const).map((l, idx) => (
                    <div key={l} className="form-field">
                      <label htmlFor={`ade-${q.id}-${idx}`}>Antwort {l} DE</label>
                      <input id={`ade-${q.id}-${idx}`} value={q.answersDe[idx] ?? ''} onChange={(e) => updateQuizAnswer(q.id, 'De', idx, e.target.value)} />
                    </div>
                  ))}
                  {(['A', 'B', 'C'] as const).map((l, idx) => (
                    <div key={l} className="form-field">
                      <label htmlFor={`afr-${q.id}-${idx}`}>Antwort {l} FR</label>
                      <input id={`afr-${q.id}-${idx}`} value={q.answersFr[idx] ?? ''} onChange={(e) => updateQuizAnswer(q.id, 'Fr', idx, e.target.value)} />
                    </div>
                  ))}
                  <div className="form-field">
                    <label htmlFor={`cor-${q.id}`}>Richtige Antwort</label>
                    <select id={`cor-${q.id}`} value={q.correctAnswerIndex} onChange={(e) => updateQuiz(q.id, 'correctAnswerIndex', Number(e.target.value))}>
                  ...
                  <div className="form-field form-field--full">
                    ...
                    <textarea id={`expde-${q.id}`} value={q.explanationDe} onChange={(e) => updateQuiz(q.id, 'explanationDe', e.target.value)} />
                  </div>
                  <div className="form-field form-field--full">
                    <label htmlFor={`expfr-${q.id}`}>Erklärung FR</label>
                    <textarea id={`expfr-${q.id}`} value={q.explanationFr} onChange={(e) => updateQuiz(q.id, 'explanationFr', e.target.value)} />
                  </div>
                </div>
